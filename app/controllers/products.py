...
class ProductController:
    ...
    def get_all_products():
        try:
            with current_app.app_context():
                ...
                if response.status_code == 200:
                    ...
                    local_ids = set(product.id for product in local_products)

                    # Ajout des produits locaux manquants dans la base de données
                    new_products = []
                    ...
                    # Mise à jour des produits locaux existants dans la base de données
                    local_products = Product.query.options(joinedload('*')).all()
        ...
        print(f"Type of first product: {type(product[0])}")
        return None
    ...
    @staticmethod
    def get_recent_products(limit=6):
        try:
            with current_app.app_context():
        ...
        try:
            with current_app.app_context():
                top_products = Product.query\
                    .order_by(desc(Product.rating_rate))\
                    .limit(limit)\
                    .options(joinedload('*'))\
                    .all()
                return top_products
    ...
    def fetch_and_save_products(count=5):
        try:
            with current_app.app_context():
                ...
                response = requests.get(ProductController.BASE_URL, params={'limit': count, 'offset': start})
                if response.status_code == 200:
